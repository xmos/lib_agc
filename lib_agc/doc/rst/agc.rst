...
The Automatic Gain Controller comprises a function that process a block of
...
``agc_block``. This function requires, in addition to the ``agc_state_t``
...
In its simplest form the AGC can be used as follows::
  ...
  while(1) {
    int32_t samples[128];
    ... get_data(samples) ...
    agc_block(a, samples,  // sample data
...
shift value for each look-ahead frame). In this case the ``agc_block``
function will delay the signal with two frames; that is, the output data
will be two frames older than the input data to agc_block::
